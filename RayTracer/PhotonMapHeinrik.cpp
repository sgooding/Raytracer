...
*/
//***********************************************
PhotonMapHeinrik :: PhotonMapHeinrik( const int max_phot ) :
    mSumFound(0.0),
    mAvgCount(0.0),
    mMax(-99999.9),
    mMin(9999.9),
...
//*************************
PhotonMapHeinrik :: ~PhotonMapHeinrik()
//*************************
{
...
void PhotonMapHeinrik :: compute_color( Vector& color,
        ...
        const int nphotons )
{
  ...
  np.pos[0] = surface_position.x; np.pos[1] = surface_position.y; np.pos[2] = surface_position.z;
  np.max = nphotons;
  np.found = 0;
  np.got_heap = 0;
  ...
  for (int i=1; i<=np.found; i++) {
    ...
    Vector L( pdir[0], pdir[1], pdir[2] );
    Vector H( (V+L).norm() );

    // Specular
    Is = Primitive.m.k_spec*pow( H.dot(surface_normal), Primitive.m.n_spec);
 
    // Diffuse
    double normal_2_ls( surface_normal.dot( L ) );
...
*/
//**********************************************
void PhotonMapHeinrik :: irradiance_estimate(
...
//***********************************************
{
  ...
  // sum irradiance from all photons
  float max_radius( 9999.0 );
  for (int i=1; i<=np.found; i++) {
    const Photon *p = np.index[i];
    /*
    std::cout << "Neighboring Distance 2 ["<<i<<"] " << std::endl;
    std::cout << np.dist2[i] << std::endl;
    */

    if( np.dist2[i] < max_radius )
    {
        max_radius  = np.dist2[i];
    }
    // the photon_dir call and following if can be omitted (for speed)
    // if the scene does not have any thin surfaces
  ...
  const float tmp=(1.0f/M_PI)/(np.dist2[0]);    // estimate of density
  //const float tmp=(1.0f/M_PI)/(max_radius);    // estimate of density
  /*
  std::cout << "Heinrik Scale : " << tmp << std::endl;
  std::cout << "Heinrik sqRad: "  << np.dist2[0] << std::endl;
  std::cout << "Heinrik sqRad: "  << max_radius << std::endl;
  abort();
  */
...
void PhotonMapHeinrik :: locate_photons(
...
//*******************************************************
void PhotonMapHeinrik :: scale_photon_power( const float scale )
//*******************************************************
{
 ...
 */
//******************************
void PhotonMapHeinrik :: balance(void)
//******************************
{
  if (stored_photons>1) {
...
void PhotonMapHeinrik :: median_split(
  Photon **p,
...
//****************************
void PhotonMapHeinrik :: balance_segment(
  Photon **pbal,
  Photon **porg,
  const int index,
  const int start,
